...
"use strict";function getRandomInt(a,t){return Math.floor(Math.random()*(t-a))+a}var COLORS=["#E6E5EB","#8ABEAF","#F1AAB4"],angular=require("angular"),app=angular.module("hendowedding");app.directive("stars",["$window",function(a){return{link:function(t,n,r){function e(){for(var a=document.body.clientWidth,t=document.body.clientHeight,r=window.devicePixelRatio,e=a*r,o=t*r,l=a*a*i*r,h=[],u=0;d>u;u++){var c=angular.element("<canvas />").addClass("layer").attr({width:a,height:t}).css({width:a,height:t});n.append(c);var g=c[0].getContext("2d");g.fillStyle=COLORS[getRandomInt(0,2)],h.push(g)}for(var u=0;l>u;u++){var m=Math.round(Math.random()*e)-.5,s=Math.round(Math.random()*o)-.5,M=Math.random();M=1.5*Math.pow(M,8),M+=.3,Math.random()<.1&&(M*=2),0>M&&(M=0);var v=1;1>M&&(v=M,M=1);var f=Math.round(Math.random()*(h.length-1)),g=h[f];g.translate(m,s),g.globalAlpha=v,g.rotate(Math.PI/4),g.fillRect(0,0,M*r,M*r),g.rotate(-Math.PI/4),g.translate(-m,-s)}}function o(){n.html("")}var d=9,i=.0012;e(),angular.element(a).on("resize",function(){o(),e()})}}}]);
